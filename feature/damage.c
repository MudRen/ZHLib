...
varargs int receive_damage(string type, int damage, object who)
{
        ...
        if (who && who->query("tianmo_jieti/times"))
                damage += damage*who->query("tianmo_jieti/times")/5;
...
void unconcious()
{
	...
	me->clear_written();
        ...
        if (me->query_temp("is_riding") && objectp(riding = present(me->query_temp("is_riding"),me)))
        {
        ...
        if (objectp(room = environment(me)) && room->query("biwu_room"))
        {
                me->move("/d/city/biwu_restroom");
