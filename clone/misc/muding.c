...
int do_liandu(string arg)
{
        ...
        {
                if (! ob[i]->is_insect()) continue;
                message_vision(HIM "$n" HIM "慢慢化作黑色血水，渗入到$N" HIM "手心。\n" NOR,
                               me, ob[i]);
	...
	if (me->can_improve_skill("poison") && SKILL_D("poison")->valid_learn(me))
	{
        	me->improve_skill("poison", amount / 2 + random(amount / 2));
        	tell_object(me,"你觉得你的基本毒技又有了新的进步。\n");
	} else tell_object(me,"你觉得你的基本毒计目前是无法再提高了。\n");

	if (me->can_improve_skill("huagong-dafa") && SKILL_D("huagong-dafa")->valid_learn(me))
	{
        	me->improve_skill("huagong-dafa", amount /2 +
						  random(amount / 2));
        	tell_object(me,"你觉得你的化功大法又有了新的进步。\n");
	} else tell_object(me,"你觉得你的化功大法目前是无法再提高了。\n");
