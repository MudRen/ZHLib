...
mixed ask_tu()
{
        ...
        if (me->add("can_perform/taiji-quan/tu", 1) < 10)
        {
                message_vision(HIY "$N" HIY "迷迷茫茫的点了"
                               "点头，深思良久。\n" NOR, me);
                tell_object(me, HIG "你对太极图有了一点领悟。\n" NOR);
        } else
        {
                ...
                tell_object(me, HIG "你学会了道家密技太极图。\n" NOR);
...
mixed ask_tao()
{
        ...
        if (me->query_skill("taiji-quan", 1) < 300)
                return "你对太极拳意的理解还不够，教了你也不会用！";

        message_vision(HIY "$n" HIY "微微一笑，双手划个圆圈，$N" HIY
                       HIY "只觉一股强大劲气将$N" HIY "困住，双臂不能动弹。\n"
                       "$p" HIY "内力一吐，$P" HIY "顿感气血翻涌，蹬蹬蹬连退数步。\n" NOR,
                       me, this_object());
        command("smile");
        command("say 你可明白了？");
        tell_object(me, HIG "你学会了太极拳法的套字诀。\n" NOR);
...
mixed ask_an()
{
        ...
        if (me->query_skill("taiji-quan", 1) < 400)
                return "「按」字诀乃太极拳法之精髓，你对太极拳意的理解还差一点！";

        message_vision(HIY "$n" HIY "哈哈一笑，双手错开，呼地一声向$N" HIY
                       HIY "压来，$N" HIY "感到一股绵气将全身困住，不觉呆在当地。\n"
                       "$p" HIY "双手向下一按，$P" HIY "几感窒息，胸中郁闷异常，"
                       HIY "不禁“哇”的喷出一口" HIR "鲜血" HIY "。\n" NOR,
                       me, this_object());
        command("smile");
        command("say 你可明白了？");
        tell_object(me, HIG "你学会了太极拳法的按字诀。\n" NOR);
