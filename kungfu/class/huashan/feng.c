...
int recognize_apprentice(object ob, string skill)
{
	...
	if (ob->query("failed_learn_jiujian"))
        {
                command("say 你没希望了，以后别来烦我了！");
                return -1;
        }

	if (ob->query("family/family_name") != "华山派")
	{
		command("say 你是哪里来的？找我干什么？还不快滚！");
		return -1;
	}
        ...
        if (ob->query("shen") == 0)
        {
                command("say 你这个人虽然没有走上邪门歪道，但是一点"
                        "正义感都没有，你走吧。");
                ob->set_temp("have_asked_feng", 1);
                return -1;
        }
	...
	if (ob->query("kar") > 22 && random(5) == 1)
	{
            ...
            log_file("static/9j_success", sprintf("%s(%s) 学习 独孤九剑 成功 at %s.\n", 
                me->name(1), me->query("id"), ctime(time()))); 
        	return 1;
	} else
	{
		command("sigh");
		command("say 我华山派剑气二宗纷争，导致现今人才凋零的局面，你还是好好的修炼的气吧。");
		command("say 独孤九剑之事，以后休再提起！\n");
            ob->set("failed_learn_jiujian",1);
            log_file("static/9j_failed", sprintf("%s(%s) 学习 独孤九剑 失败 at %s.\n", 
                me->name(1), me->query("id"), ctime(time()))); 
		return -1;
	}
}
...
int accept_ask(object me, string topic)
{
        ...
        {
                command("say 你不是已经用" + topic + "了么？还来问我干什么？");
                return 1;
        }
