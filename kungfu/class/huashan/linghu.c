...
int accept_object (object who, object ob)
{
       ...
       if (ob->query("id") == "qing xin")
       {
            if (who->query_temp("ren_qingxin"))
            {
                  who->delete_temp("ren_qingxin");
            ...
            {
                  message_vision("$N接过信看了看，又抬头把$n打量了一番，道：“"
                                 "这信你是从哪搞到的？”\n", me, who);
                  return 1;
            }
       }
...
void read_letter(object me, object who, object ob)
{
       object obn = new("/d/heimuya/npc/obj/card2");

       destruct(ob);
       message_vision("$N看完信，不由得一声长叹。\n"
                      "$N道：多谢这位" + RANK_D->query_respect(who) +
                      "把信带到，我无以为报，这块令牌是盈盈交给我"
                      "的，你拿去吧，也许对你有点用。\n", me);
       obn->move(me);
       command("give card2 to " + who->query("id"));
}
