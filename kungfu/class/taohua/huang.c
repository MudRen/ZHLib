...
void create()
{
        ...
        set_skill("luoying-shenzhang", 220); // 落英神剑掌
	...
	set_skill("throwing", 270);	     // 基本暗器
        ...
        set_skill("luoying-shenfa",250);
        set_skill("yuxiao-jian", 250);       // 玉箫剑法
        set_skill("lanhua-shou", 270);       // 兰花拂穴手
        set_skill("qimen-wuxing", 220);      // 奇门五行
        set_skill("count", 220);             // 阴阳八卦
...
int ask_bagua()
{
	...
	{
		command("say 你的武功还需要更进一步，不要这么着急。\n");
		return 1;
	}

	if (present("tie bagua",me))
	{
		command("say 你身上不是已经有了一块了么？怎么还想要啊？\n");
		return 1;
	}

	if (!query("bagua_count"))
	{
		command("say 铁八卦都给别的弟子要去了，现在是没有了。\n");
		return 1;
	}
	...
	tell_object(me,"黄药师把一块" + ob->query("name") + "给了你：这可是桃花岛的必备法宝，小心照管，别弄丢了。\n");
	return 1;
}
...
mixed ask_qing()
{
        ...
        message_vision(HIC "$n" HIC "举头望天，若有所思，随手搭在$N"
                       HIC "的肩头，$N" HIC "忽然发现内力源源而逝，不由大惊！\n" NOR,
                       me, this_object());
        command("sigh");
        command("say 你懂了么？");
        tell_object(me, HIG "你通晓了天地情长的奥妙。\n" NOR);
...
int accept_object(object who, object ob)
{
	...
	if (r = ::accept_object(who, ob))
        ...
        if (ob->query("id") == "jiuyin zhenjing1")
        {
                destruct(ob);
                command("sigh");
