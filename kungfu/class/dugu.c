...
//   message_vision("$N将$n交给独孤求败。\n",who,ob); 与give信息重复 haiyan
   ...
   if (quest_count == 50)
   {
       msg = "$N呵呵笑了两声道：“你还可以，连着"+chinese_number(who->query_temp("questdg_times"))+
       "次任务都完成的漂漂亮亮，继续努力！”\n";
	special = 1;
	   } else
       if (quest_count == 100)
   { 
       msg = "$N笑的合不拢嘴，看了你一眼道：“有志气！"+chinese_number(who->query_temp("questdg_times"))+
       "次任务干得干净利索！”\n";
       special = "/d/shaolin/obj/puti-zi";
   } else
    if (quest_count == 150)
    {
        msg = "$N叹道：“真是长江后浪推前浪，想不到你接连"+chinese_number(who->query_temp("questdg_times"))+
               "次任务都无一疏漏，不易，不易！”\n";
        money = 1;
    } else
    if (quest_count == 200)
    ...
    if (quest_count == 350)
    {
        msg = "$N吓了一跳：“看来你真的很适合学武，竟接连"+chinese_number(who->query_temp("questdg_times"))+
               "次任务都无一疏漏，不易，不易！”\n";
	 special = "/clone/misc/tianxiang";
    } else
    if (quest_count == 400)
    {
        msg = "$N叹道：“真是一个武学天才啊，想不到你接连"+chinese_number(who->query_temp("questdg_times"))+
               "次任务都无一疏漏，不易，不易！”\n";
	 special = "/quest/obj/fig";
    } else
    if (quest_count == 450)
        {
        msg = "$N大声叫起来：“天才！真是天才，真有人连续"+chinese_number(who->query_temp("questdg_times"))+
               "次任务都无一疏漏，不易，不易！”\n";
        special = "/clone/fam/max/longjia";
    } else 
    if (quest_count == 0)
    {
       msg = "$N深深叹了口气，道：“没想到你连着"+chinese_number(who->query_temp("questdg_times"))+
...
//                obj->move("/d/wizard/prison"); 出错，不知何意，暂取消 haiyan
                destruct(obj);
            }
