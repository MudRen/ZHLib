...
int recognize_apprentice(object ob, string skill)
{
	...
	if (skill == "kurong-changong" && ob->query("can_learn/kurong-changong"))
		return 1;
...
mixed ask_kurong()
{
	object me;
	me = this_player();
	if (me->query("can_learn/kurong-changong"))
	{
		command("say 你好好把枯荣禅功学成，这可是无上心法！\n");
		return 1;
	}

	if (me->query("family/family_name") != "段氏皇族")
	{
		command("say 不是老衲藏私，枯荣禅功如果非段氏皇族成员修炼，多半会走火入魔。\n");
		return 1;
	}

	if (me->query_skill("duanshi-xinfa",1) < 200)
	{
		command("say 你的段氏心法还修炼不够，如果强练枯荣禅功，对你有害无益！\n");
		return 1;
	}

	command("say 好吧，我就将天龙寺绝学枯荣禅功传授给你！\n");
	me->set("can_learn/kurong-changong",1);
	return 1;
}