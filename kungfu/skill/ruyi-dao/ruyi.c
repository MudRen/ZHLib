...
int perform(object me, object target)
{
	...
	if ((int)me->query_skill("xiaowuxiang", 1)<200
		&& (int)me->query_skill("lingjiu-xinfa", 1)<200
		&& (int)me->query_skill("xiaoyao-xinfa", 1)<200
		&& (int)me->query_skill("bahuang-gong", 1)<200
        ...
        if (ap/2 + random(ap) > dp || !living(target))
	{
		...
		damage = me->query_skill("ruyi-dao",1) + me->query("jiali") + me->query_str()*10;
                ...
                if (me->query_skill_mapped("force") == "beiming-shengong" && target->query("neili") > me->query_skill("force") * 2)
                        target->add("neili",-me->query_skill("force")*2);
