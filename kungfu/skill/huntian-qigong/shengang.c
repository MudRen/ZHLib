...
*/
	...
	skill1 /= 3;
        ...
        weapon->add("weapon_prop/damage", skill1);
...
void checking(object me, object weapon)
{
        ...
        if ( me->query_temp("shengang") < (int)me->query_skill("force")/8 ) {
           ...
           remove_effect(me, weapon);
           message_vision(HIR "\n$N的混天气功运行完毕，将内力收回丹田，$n"HIR"上的淡光亦消失无踪。\n\n" NOR, me, weapon);
        }
}       
...
private int remove_effect(object me, object weapon)
{
        ...
        if ( weapon == me->query_temp("weapon") || weapon == me->query_temp("secondary_weapon") ) {
                weapon->unequip();
                me->set_temp("equip_weapon", 1);
        }
        
        weapon->add("weapon_prop/damage", -amount1 );
