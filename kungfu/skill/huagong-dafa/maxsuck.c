...
//********************************************************************************
        ...
        if (me->query("potential") > me->query_potential_limit()) 
                return notify_fail("你的潜能太多了，无法再靠吸取丹元增加潜能。\n"); 
        ...
        if( (int)target->query("max_neili") < 100
         ...
         || (int)target->query("jingli") < (int)target->query("max_jingli") / 3 )
                return notify_fail( target->name() +
                        "丹元涣散，功力未聚，你无法从他体内吸取任何东西！\n");

        if( (int)target->query("combat_exp") < (int)me->query("combat_exp") * 2 / 3)
                return notify_fail( target->name() +
                        "的武功修为远不如你，你无法从他体内吸取丹元！\n");
        ...
        dp = target->query_skill("force") + target->query_skill("dodge") + me->query("kar")*50;
...
// Allow them a little bit higher than limit (*10)
        my_max = me->query_neili_limit();
        my_max2 = me->query_jingli_limit() ;
