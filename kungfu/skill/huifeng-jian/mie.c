...
int perform(object me, object target)
{
        ...
        if( me->query_skill("huifeng-jian", 1) < 100 )
                return notify_fail("你回风拂柳剑法修为还不够，还未领悟「灭剑」！\n");

        if( me->query_skill("linji-zhuang", 1) < 100 )
                return notify_fail("你的内功修为火候未到，施展「灭a剑」只会伤及自身！\n");
        ...
        { // the target is already being performed by mie
                ...
                // but 100% succ rate !!
                busy_time = me->query_skill("huifeng-jian", 1) / 50;
        ...
        else if( me->query_temp("miejian/"+target->name()) )
        {
                ...
                call_out("remove_effect1", sword/3, me, target, dodge, parry, attack, sword);
        }
        else
        {
                message_vision(HIY "\n$N一声清啸，身形腾空丈许，将峨嵋绝技「灭剑」心法"+
...
void remove_effect1(object me, object victim, int dodge, int parry, int attack, int sword)
{
        if( objectp(victim) )
        {
                victim->add_temp("perform_target/miejian", -1);
                ...
                victim->add_temp("apply/attack", attack/3);
                victim->add_temp("apply/speed", dodge/30);
        }
