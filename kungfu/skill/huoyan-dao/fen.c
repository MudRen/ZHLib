...
int perform(object me, object target)
{
        ...
        message_vision(HIR "$N长吸一口气，两掌变得通红，射出一股灼人热气。$n的身影被笼罩在一团火焰之中。\n\n" NOR, me,target);
        lvl = (int)(lvl / 5);
        me->add("neili", -lvl*2); 
        me->add("jingli", -100);  
...
/*
ap = COMBAT_D->skill_power(me,"strike", SKILL_USAGE_ATTACK)/2;
...
//      ap = 41525168 , pp = 32932532 , dp = 29758313 .
//          ap = 41074584 , pp = 32932532 , dp = 29758313 .
        if (ap/2 + random(ap) > dp) {
            tell_object(target, RED"你只觉一股热浪扑面，霍的胸口一阵剧痛。\n"NOR);
            if ( target->query_temp("armor/cloth") && me->query_temp("honglian") ){
                armor = target->query_temp("armor/cloth");
                if( armor->query("armor_prop/armor") < 150){
                
                if( armor->query("id") == "armor" && !armor->is_item_make() )
                     ...
                     armor->set_name("破碎的" + armor->query("name"));
                     armor->set("value", 0);
                     armor->set("armor_prop/armor", 0);
                     target->reset_action();
                }
            }
            dmg = me->query_skill("force")+me->query_skill("strike")+me->query("jiali");
            damage = dmg + random(dmg);
            max_dmg = 2000;
            ...
            if (ap/2 + random(ap) < pp) damage = random(damage);
