...
int perform(object me, object target)
{
  ...
  if (living(target))
  if (random(me->query("combat_exp")) * me->query_skill("staff") > 
...
int effect_one(object me, object target, object weapon, int succeed)
{
  ...
  if(succeed) {
    ...
    target->add("jing",-me->query_skill("staff")/10);
    target->add("jingli",-me->query_skill("staff")/10);
...
int effect_two(object me, object target, object weapon, int succeed)
{
  ...
  if(succeed) {
    ...
    if(me->is_fighting() && target->is_fighting()) 
     ...
     else call_out("remove_effect",zhuan_duration-1,me,target,zhuan_busy);
