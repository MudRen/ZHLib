...
int perform(object me, object target)
{
        ...
        if (! living(target))
                return notify_fail("人家都已经这样了，无需再施展绊字诀！\n");
        ...
        if (!weapon) return notify_fail("你没有装备兵器。\n");
        if (weapon->query("skill_type") != "staff") return notify_fail("你装备的兵器不对。\n");

        if( me->query_skill("staff") < 300 ) { hits=3; }
        else if( me->query_skill("staff") < 400 ) { hits=4; }
        else if( me->query_skill("staff") < 500 ) { hits=5; }
        else { hits=6; }

                if( random(2)) {
                        message_vision(HIG "突然$N"+ weapon->name()+HIG"挥出，以打狗棒的「绊」字诀向$n着地扫去，有如长江大河，绵绵而至。\n\n" NOR, me, target); } 
                else {
