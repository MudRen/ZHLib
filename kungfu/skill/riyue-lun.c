...
mapping query_action(object me, object weapon)
{
        ...
        if ( me->query_skill_mapped("force") == "longxiang" &&
            objectp(ob) &&
            ((string)ob->query("name") == "法轮" ||
             (string)ob->query("name") == HIY"金轮"NOR ||
             (ob->is_item_make() && ob->query("skill_type") == "hammer") )){
        if (random(me->query_skill("hammer",1)) > 275 
        && me->query("neili") > 600 && (ob->is_item_make() || ob->query_amount() > 4 )){
        me->add("neili", -50);
        return ([
                "action":HIY "锵锵一阵声响，$N使出「"+(ob->is_item_make()?"五":chinese_number(ob->query_amount()))+"轮大转」，运轮如飞，空中飞起数只"+NOR+"$w"+NOR+HIY+"，双手仍各握一轮，一齐砸向$n"NOR,
        ...
        if (random(me->query_skill("hammer",1)) > 180 
        && me->query("neili") > 300 && (ob->is_item_make() || ob->query_amount() > 1 )){
        me->add("neili", -30);
                return ([
                "action":HIW "$N两只手中"+(ob->is_item_make()?"两":chinese_number(ob->query_amount()))+"只"+NOR+"$w"+NOR+HIW+"对击，声若龙吟，悠悠不绝，左手袍袖挥处，右手"+NOR+"$w"+NOR+HIW+"砸了出去"NOR,
        ...
        if (random(me->query_skill("hammer",1)) > 150
        && me->query("neili") > 200 && (ob->is_item_make() || ob->query_amount() > 2 )){
        return ([      
        "action":HIC "$N掷出手中全部"+(ob->is_item_make()?"数":chinese_number(ob->query_amount()))+"只"+NOR+"$w"+NOR+HIC+"，一式「显吉祥」，或直飞、或斜飞、或回旋、或下坠、或上升，霎时间$n的全部退路已被封死"NOR,
        "force" : 450+random(force),
        "dodge" : -5,
        "parry" : 25,
