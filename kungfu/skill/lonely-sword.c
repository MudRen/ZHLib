...
mixed valid_damage(object me, object victim, int damage, object the_weapon)
{
	...
	ap /= 100;
        ...
        if ( random(ap+dp) > ap ) {
                result = HIG "$n突然举剑一刺，剑招随便无章，却莫明奇妙地突破了$N的攻势，大出$N意料！\n";
		...
		victim->add("jingli", -30);
                ...
                result = HIG "$n举剑反攻，后发先至地刺向$N，不仅把$N招术封死，更进一步发出攻击！\n" NOR;
		victim->add("neili", -20);
		...
		me->start_busy( level/150 );
                ...
                result = HIG "$n提剑回刺，落点正是$N的破绽，$N不得不撤招严守，身形一晃间已退出丈许！\n" NOR;
		victim->add("jingli", -20);
                damage = -damage;
                ...
                result = HIG "$n眼见避无可避，提剑毫无章法乱刺一通，却出乎意料地将$N的攻击接去大半。\n" NOR;
