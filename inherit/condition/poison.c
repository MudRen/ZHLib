...
string *query_valid_force() { return ({}); }
...
int dispel(object me, object ob, mapping cnd)
{
	...
	my_force = me->query_skill_mapped("force");
	valid_force = query_valid_force();

	if (sizeof(valid_force))
	{
		if (member_array(my_force,valid_force) == -1)
		{
			tell_object(me,"你的" + to_chinese(my_force) + "无法驱散" + pos + "。\n");
			return -1;
		}
	}
