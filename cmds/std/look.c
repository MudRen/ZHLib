...
HIY"返璞归真"NOR,HIY"独步天下"NOR,HIY"旷古绝伦"NOR,HIY"震慑天下"NOR,HIR"纵横中华"NOR,
});
        ...
        if (mapp(exits = env->query("exits")))
	{
        ...
        if (me->query("env/way"))
        {
               ...
               {                       
                       for (i = 0; i < sizeof(dirs); i ++)
                       {
                              if (! next_room = find_object(exits[dirs[i]]))
                                          next_room = load_object(exits[dirs[i]]);

                              if (! objectp(next_room))continue;

                              str += sprintf(HIC "%-50s", stringp(ways[dirs[i]]) ?
                                      HIR + ways[dirs[i]] + HIC + filter_color(next_room->query("short")) + 
                                      "(" + dirs[i] + ")" NOR :
                                      filter_color(next_room->query("short")) + "(" + dirs[i] + ")" NOR);
                              if (i % 2)str += "\n    ";
                       }
               }
               str += "\n";
