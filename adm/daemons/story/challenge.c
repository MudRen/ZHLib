...
string prompt() { return HIR "【卫我华夏】" NOR; }
...
mixed master_opinion(int n)
{
        ...
        if (! sizeof(u))
        {
                if (! too_cheap)
                {
                        ob->force_me("chat 可惜啊可惜，现在中华诸多英豪，却也"
                                     "无人能收拾此人！");
                ...
                ob->force_me("chat 哈哈！中华英雄辈出，能收拾你的人数不胜数！就你这"
                             "点三脚猫功夫，还不赶紧滚远点！");
        ...
        if (sizeof(u) < 10)
        {
                ...
                ob->force_me("chat 想我中华能收拾此人的固然不少，不过合适"
                             "的确是寥寥无几。");
        } else
        {
                ob->force_me("chat* heng");
                ob->force_me("chat 这人自不量力！中华能摆平他的英豪数不胜数！");
        }
...
mixed continue_story(int n)
{
        ...
        case NO_USER_CAN_RESPOND:
			...
			"rumor", "听说来自" + cob->query("nation") +
                        "的" + cob->name() + "在中华居然没有对手，真是"
                        "中华武林的奇耻大辱！");
                return cob->name() + "：“中华武林，不过如此！我去了！”";
        ...
        case CHALLENGER_IS_FIGHTING:
			...
			"rumor", "听说" + accepted + "在和" + cob->query("nation") +
                        "" + cob->name() + "交手的时候临阵脱逃，使中华武林蒙羞！");
                        return cob->name() + "狂笑不止：“懦夫！华夏都是一些懦夫！哈哈！”";
                }
...
string cannot_accept(object ob)
{
        ...
        if (ob->query("combat_exp") < cob->query("combat_exp") / 2)
                return "你的武功和挑战者相比差的太远，还是不要丢我们中华"
                       "武林的脸了！\n";;
...
void accept_for_challenge(object me)
{
        ...
        me->force_me("chat " + cob->name() + "休得猖狂，我来了！");
