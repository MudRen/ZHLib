...
void done_post(object me, mapping note, int n, string text)
{
        ...
        message_system(HIW + me->name(1) + HIW "发布了一条新闻！\n" NOR);
        save();
}
...
void do_post(object me, string arg)
{
        ...
        if ((! wizardp(me) || ! interactive(me)) &&
            (! mapp(me->query("viremploy")) || ! interactive(me)))
        {
        ...
        note["title"] = arg;
        if (wizardp(me) || ! stringp(me->query("viremploy/job")) )
                   note["author"] = me->name(1) + "-" + me->query("id");
        else 
                   note["author"] = me->query("viremploy/job") + "：" + 
