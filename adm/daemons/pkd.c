...
int join_competition(object ob)
{
        ...
        if (ob->query_condition("killer",1))
                return notify_fail("你正在被官府，所以不能进入屠人场。\n");

        if (ob->query_condition())
                return notify_fail("请确定身上的毒已经清除、身体状况正常且吃饱喝足了再进入屠人场。\n");
...
private int start_competition()
{
        ...
        if (sizeof(obs) == 2)
        {
                if (obs[0]->query("couple/couple_id") == obs[1]->query("id"))
                {
...
private void auto_check()
{
        ...
        if (sizeof(total) == 2 && total[0]->query("couple/couple_id") == total[1]->query("id"))
        {
                msg = "听说本次屠人大赛圆满结束，" +
...
string reject_join(object me)
{
        ...
        if (me->query_condition("killer",1))
                return "你正在被官府，所以不能参加屠人大赛。";

        if (me->query_condition())
                return "请确定身上的毒已经清除、身体状况正常且吃饱喝足了再参加屠人大赛。";
