...
// 以下为特殊装备增加的一个函数 yuchang
varargs void report_status(object ob, int effective)    
{
    if( effective ) 
        message_vision( "( $N" + eff_status_msg(
                    (int)ob->query("eff_qi") * 100 /(1+(int)ob->query("max_qi")) )
                   + " )\n", ob);
    else
...
int attack_spells(object me,object victim,object weapon,string spells_kind)
{
	...
	k=random(grade)+1;
	if( k>6 )  //封存武器终极招式，yuchang 2004/11
	k=6;
	   ...
	   switch(spells_kind)
	   {
	     ...
	     case "fire_attack"://火系魔法：打气血或气血上限
        	...
        	victim->receive_damage("qi", damage_spells*2, me );
                  ...
                  {
        	victim->receive_wound("qi", damage_spells*3, me );
	     ...
	     case "lightning_attack"://电系魔法：打精神或精神上限
        	...
        	if ( random(10)>3 || me->query_temp("apply/lightning_attack")==1)
        	  {
        	victim->receive_damage("jing", damage_spells*3, me );
	     ...
	     case "poison_attack"://毒系魔法：打内力或者降防御
        	...
        	if ( random(10)>4 || me->query_temp("apply/poison_attack")==1)
        	  {
                victim->add("neili", -damage_spells*2);
