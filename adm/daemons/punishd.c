                ...
                {            
                        ...
                        if (obs[i]->query("combat/WPK") > WANT_PKS || 
                            obs[i]->query("combat/PKS") > ALL_PKS)
                                  obs[i]->apply_condition("pk", 7200); // 24小时

                        if (! obs[i]->query_condition("pk"))continue;

                        obs[i]->set("startroom", PK_PUNISH);
                        tell_object(obs[i], HIC "执法精灵告诉你：“你PK过度了，跟我走吧！”\n" NOR);
                        tell_object(obs[i], HIG "猛然间你感觉头一昏 ……\n" NOR);
                        obs[i]->move(PK_PUNISH);
                        CHANNEL_D->do_channel(this_object(), "rumor",
                                              "听说" + obs[i]->name(1) + HIM "由于PK过度，被禁闭在" HIW 
                                              "通天岛" HIM "二十四小时。\n" NOR);
                        
                        log_file("pk_punish", sprintf("%s(%s) 在 %s 由于PK过度被禁闭24小时。\n",
                                                      obs[i]->name(), 
                                                      obs[i]->query("id"), 
                                                      ctime(time())));
                }
