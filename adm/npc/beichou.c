...
void init()
{
        ...
        command("say 新玩家可以找我拿礼物！(ask bei about 中华英雄）");
}
...
string give_gift()
{
        ...
        // 已经领取过礼物
        if (me->query("get_moon_gift/y2003"))
                 return HIR "“祝你在中华英雄玩得开心！”\n" NOR;
        ...
        // 给予礼物
        message_vision(HIY "北丑笑嘻嘻地对$N说道：“祝你开心！”\n" NOR, me);
        command("smile");
        command("say 好了，你的礼物，给你吧！");
        message_vision(HIY "北丑拿出一颗浪翻云特制的金丹给$N" HIY "\n", me);        
        tell_object(me, HIG "你得到金丹，快吃了它吧！\n");
        

        gift = new("/u/yuchang/gift/yuebing");
        gift->move(me, 1);
       
        me->set("get_moon_gift/y2003", 1);
        return "Bye！\n";
...
int cancel_uquest()
{
        ...
        if (! me->query_temp("cancel_quest"))
        {
	       command("smile");
	       command("say 取消宗师任务需要一两黄金，如果你决定了请再来找我！");
 	       me->set_temp("cancel_quest", 1);
 	       return 1;
 	}
 	if (me->query("balance") < 10000)
 	{
 		command("shake");
 		command("say 既然你没那么多钱，我也不能帮你了！");
 	}
 	else
 	{
 		command("say 好吧，既然你愿意出钱，那我就帮你一次吧！");
 		me->add("balance", -10000);	
 		me->delete_temp("cancel_quest");
 		me->delete("ultra_quest");
 		me->save();
 	}
	return 1;
}
