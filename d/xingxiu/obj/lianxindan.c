...
int throw_ob(object me, object victim)
{
   ...
   victim->affect_by("insect_poison",
   ([ "level" : me->query_skill("poison",1) + random(me->query_skill("poison",1)/2),
      "id"    : me->query("id"),
      "duration" : me->query_skill("poison",1)/50 + random(me->query_skill("poison",1)/50)]));
   victim->set("poisoner", me);
   victim->receive_wound("qi", 100, me);
   victim->receive_wound("jing", 50, me);
