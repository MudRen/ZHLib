...
int do_dj()
{
        ...
        if (slsf < 180)
                  return notify_fail("你的少林身法还不到家，无法使出"HIY"[一苇渡江]"NOR"。\n");

        if (me->query("neili") < 100)
                  return notify_fail(HIR"以你现在的状态用"HIY"[一苇渡江]"HIR"渡水太危险！你还是做船过去吧。\n"NOR);

        if (me->is_busy())
                  return notify_fail("你现在正忙着，无法渡江。\n");

        if (slsf >= 180){
        write(HIY"你使出少林绝技[一苇渡江]，身子轻轻一跃已经准确无误的踩上了一根芦苇，然后迅速的向对岸漂去！\n"NOR);
        me->move("/d/shaolin/hanshui2");
        me->add("neili",-50);
        tell_room("/d/shaolin/hanshui2.c",HIG+me->query("name")+"脚上踩着一根芦苇快速的漂了过来！\n"NOR);
        return 1;
        }
}
